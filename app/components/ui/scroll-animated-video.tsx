...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
    ...
    const getEmbedUrl = (url: string) => {
        if (url.includes("vimeo.com")) {
            ...
            return `https://player.vimeo.com/video/${vimeoId}?background=1&autoplay=1&loop=1&byline=0&title=0&muted=1`;
        }
        if (url.includes("youtube.com") || url.includes("youtu.be")) {
            // simplified logic, assumes standard watch URL or share URL
            let id = "";
            if (url.includes("youtu.be")) id = url.split("/").pop() ?? "";
            else id = new URLSearchParams(url.split("?")[1]).get("v") ?? "";
            return `https://www.youtube.com/embed/${id}?autoplay=1&mute=1&controls=0&loop=1&playlist=${id}&playsinline=1&enablejsapi=1`;
        }
        return null;
    };
                ...
                return (
                    <iframe
                        ...
                        style={{
                            ...
                            position: "absolute",
                            inset: 0,
                            pointerEvents: "none", // ensure scroll works over it
                        }}
                    />
                );
            }
